...
def show_add_recipe_screen(db_conn, username, user_id):
    ...
    recipe_type_input.set("Breakfast")
    ...
    recipe_cook_time_input.set("Under 30 Minutes")
    ...
    def add_recipe():
        ...
        recipe_was_added = False

        if input_is_empty(recipe_name):
            messagebox.showwarning("No Recipe Name", "Please Enter a Recipe Name")
        elif input_is_empty(recipe_ingredients):
            messagebox.showwarning("No Recipe Ingredients", "Please Enter the Recipe Ingredients")
        elif input_is_empty(recipe_cooking_method):
            messagebox.showwarning("No Recipe Cooking Method", "Please Enter the Recipe Cooking Method")
        else:
            cursor = db_conn.cursor()
            cursor.execute('''INSERT INTO recipes (recipe_name, recipe_type, recipe_ingredients, recipe_cooking_method, 
                            recipe_cook_time, recipe_is_shareable) VALUES (?, ?, ?, ?, ?, ?)''',
                           (recipe_name, recipe_type, recipe_ingredients, recipe_cooking_method, cook_time, is_shareable))
            db_conn.commit()
            cursor.close()
            messagebox.showinfo("Success", f"{recipe_name} was successfully added!")
